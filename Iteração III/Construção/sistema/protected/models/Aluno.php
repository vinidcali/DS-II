...
class Aluno extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('curso', 'length', 'max'=>30),
			array('matricula', 'length', 'max'=>20),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, curso, matricula', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'pessoa' => array(self::BELONGS_TO, 'Pessoa', 'id'),
			'frequencias' => array(self::HAS_MANY, 'Frequencia', 'aluno_id'),
			'inscricaos' => array(self::HAS_MANY, 'Inscricao', 'aluno_id'),
			'mediases' => array(self::HAS_MANY, 'Medias', 'aluno_id'),
			'resultadoses' => array(self::HAS_MANY, 'Resultados', 'aluno_id'),
		);
	}
